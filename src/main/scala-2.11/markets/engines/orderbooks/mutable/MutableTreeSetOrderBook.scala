...
/** Class implementing the MutableOrderBook interface using a mutable TreeSet as the underlying
  ...
  */
class MutableTreeSetOrderBook[A <: Order](val ordering: Ordering[A])
  extends MutableOrderBook[A, mutable.TreeSet[A]] with SortedOrderBook[A, mutable.TreeSet[A]] {

  /** Add an order to the order book.
    *
    * @param order the order that is to be added to the order book.
    */
  def add(order: A): Unit = backingStore.add(order)
  ...
  /** Remove an order from the order book.
    ...
    */
  def remove(order: A): Boolean = backingStore.remove(order)
...
object MutableTreeSetOrderBook {
  ...
  def apply[A <: Order](ordering: Ordering[A]): MutableTreeSetOrderBook[A] = {
    new MutableTreeSetOrderBook(ordering)
  }
