...
/** Actor for modeling market clearing mechanisms.
  *
  * A `ClearingMechanismActor` actor should receive orders and fill them using its matching engine. Filled
  * orders are then sent to a `SettlementMechanismLike` actor for further processing.
  * @param matchingEngine A `ClearingMechanismActor` has a matching engine for forming prices and quantities
  * @param settlementMechanism A `ClearingMechanismActor` has access to some settlement mechanism that it uses to
  *                            process filled orders into successful transactions
  */
