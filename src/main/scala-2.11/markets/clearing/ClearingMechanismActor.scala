...
/** Base implementation of a `ClearingMechanismActor`.
  *
  * @note Users should always be able to create a `ClearingMechanismLike` actor directly using this class. For
  *       convenience, many standard clearing mechanisms should inherit from this class.
  */
class ClearingMechanismActor(val matchingEngine: MatchingEngineLike,
                             val settlementMechanism: ActorRef) extends Actor with ClearingMechanismLike
