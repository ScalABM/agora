...
class SortedBidOrderBook[B <: BidOrder](tradable: Tradable)(implicit ordering: Ordering[B])
  extends BidOrderBook[B](tradable) with MutableSortedOrders[B] {
...
object SortedBidOrderBook {
  ...
  /** Create a `SortedBidOrderBook` instance for a particular `Tradable`.
    ...
    */
  def apply[B <: BidOrder](tradable: Tradable): SortedBidOrderBook[B] = new SortedBidOrderBook[B](tradable)
