...
/** Trait defining a `PostedPriceAuction`.
  ...
  * @tparam CC the type of collection class used to store
  */
trait PostedPriceAuctionLike[O <: Order with LimitPrice with Persistent with Quantity, CC <: collection.GenIterable[(UUID, O)]] {
  ...
  protected def orderBook: orderbooks.GenOrderBook[O, CC]
