...
class PriorityOrderBook[O <: Order, +CC <: mutable.Map[UUID, O]](val tradable: Tradable)(implicit priority: Ordering[O], cbf: CanBuildFrom[_, _, CC])
  extends org.economicsl.agora.markets.auctions.orderbooks.OrderBookLike[O] with ExistingOrders[O, CC] with PrioritisedOrders[O] {
  ...
  /** Reduces the existing orders of this `PriorityOrderBook`, if any, using the specified associative binary operator.
    ...
    */
  def reduce[O1 >: O](op: (O1, O1) => O1): Option[O1] = prioritisedOrders.reduceOption(op)
