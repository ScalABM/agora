...
import org.economicsl.agora.orderbooks.mutable.{ExistingOrders, OrderBook, SortedOrders}
import org.economicsl.agora.tradables.orders.{NonPriceCriteria, Order, PriceCriteria}
...
/** Class defining a `MatchingFunction` that finds the `Order` with the "best price" in a `SortedOrderBook`.
  ...
  */
class FindBestPricedMatch[-O1 <: Order with PriceCriteria[O2] with NonPriceCriteria[O2], O2 <: Order]
  extends MatchingFunction[O1, OrderBook[O2] with ExistingOrders[O2] with SortedOrders[O2], O2] {
  ...
  /** Matches a given `Order` with the first acceptable `Order` found in some `SortedOrderBook`.
    ...
    */
  def apply(order: O1, orderBook: OrderBook[O2] with ExistingOrders[O2] with SortedOrders[O2]): Option[O2] = {
