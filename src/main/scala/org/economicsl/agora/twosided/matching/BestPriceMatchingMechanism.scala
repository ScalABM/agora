...
class BestPriceMatchingMechanism[A <: AskOrder with PriceCriteria[B] with NonPriceCriteria[B],
                                B <: BidOrder with PriceCriteria[A] with NonPriceCriteria[A]]
  extends MatchingMechanism[A, orderbooks.mutable.SortedOrderBook[A, mutable.Map[UUID, A]], B, orderbooks.mutable.SortedOrderBook[B, mutable.Map[UUID, B]]] {

  /** One-side matching function used to match an `AskOrder` with an order book containing `BidOrder` instances. */
  val askOrderMatchingFunction = new onesided.matching.FindBestPricedMatch[A, B]()

  /** One-side matching function used to match a `BidOrder` with an order book containing `AskOrder` instances. */
  val bidOrderMatchingFunction = new onesided.matching.FindBestPricedMatch[B, A]()

}


object BestPriceMatchingMechanism {

  def apply[A <: AskOrder with PriceCriteria[B] with NonPriceCriteria[B],
            B <: BidOrder with PriceCriteria[A] with NonPriceCriteria[A]](): BestPriceMatchingMechanism[A, B] = {
    new BestPriceMatchingMechanism[A, B]()
  }
