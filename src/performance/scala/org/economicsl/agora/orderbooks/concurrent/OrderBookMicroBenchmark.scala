...
*/
package org.economicsl.agora.orderbooks.concurrent

import org.economicsl.agora.OrderGenerator
import org.economicsl.agora.tradables.orders.ask.{AskOrder, LimitAskOrder}
...
object OrderBookMicroBenchmark extends Bench.OnlineRegressionReport with OrderGenerator {
  ...
  performance of "OrderBook" config (
    ...
    measure method "add" in {
      using(orderBooks) in {
        orderBook =>
          val newOrder = orderGenerator.nextAskOrder(0.5, None, tradable=validTradable)
