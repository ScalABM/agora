...
/** Simulation used to benchmark the scaling performance of the various default dispatchers used
  * by Akka to manage the concurrency.
  *
  */
object AkkaDefaultDispatcherBenchmarkSimulation extends App {

  val config = ConfigFactory.load("akkaDefaultDispatcherBenchmarkSimulation.conf")

  config.resolveWith()

  val system = ActorSystem("AkkaDefaultDispatcherBenchmarkSimulation", config)
